...
public class nds63_UnoPlayer implements UnoPlayer {
	...
	public int play(List<Card> hand, Card upCard, Color calledColor, GameState state){
		...
		for (int score = 0; score < totalScoreOfUpcomingPlayers.length; score++){
			//first, find out who has the highest score in the overall Uno match so we can use our
			...
			if (totalScoreOfUpcomingPlayers[score] > highestScore){
				highestScore = totalScoreOfUpcomingPlayers[score];
		...
		for (int i = 0; i < hand.size(); i++){
			...
			if (hand.get(i).getColor() != recentColor){
				if (hand.get(i).getRank() == Rank.NUMBER && hand.get(i).getNumber() == numOfUpCard){//Since we want to play a different color from the last color called from the current overall winner,
					return i;																		//we can do this by trying to use a number that is a different color from the last called color increasing the chance
																									//that he or she must draw another card instead of playing one.
				}
			}
			/*If we cannot change the color with numbers, then I can at least try and change the color using a wild draw four
			 * or a regular wild or a drawTwo card if I have to my advantage because then I will call the color based on what I need and hopefully not
			 * the same color as the current overall winner.
			 */
			else if(hand.get(i).getRank() == wildDrawFour){
				return i;
			}
			
			else if(hand.get(i).getRank() == wild){
				return i;
			}
		}
		...
		if (numCardsInHandsOfUpcomingPlayers[0] < 3){
			
			for (int a = 0; a < hand.size(); a++){
				
				if (hand.get(a).getColor() == colorOfUpCard){
					
					if (hand.get(a).getRank() == skip){//If the upcoming player is close to winning with one or two cards left, then use a skip or reverse if I have it
						return a;					   //so we can delay this player a turn and hopefully increase my chance of winning
					}
					if (hand.get(a).getRank() == reverse){//Same idea but use a reverse instead if I have one in my hand
						return a;
					}
					if(hand.get(a).getRank() == drawTwo){//Same thing with a drawTwo card
						return a;
					}
				}
				
				if(hand.get(a).getRank() == wildDrawFour){
					return a;
				}
		...
		for (int z = 0; z < hand.size(); z++){
			if (playedActionCards > 15){//If the action cards are more than half gone, then we it is ideal to save the action cards toward the
										//end since they are easier to get rid of.  This also reduces the chance of us losing with action cards
										//in our hand since they are worth more points.  But since 'half' of the cards were already used, it is 
										//smart to save them for the end or when we need them the most.
				if(hand.get(z).getRank() == Rank.NUMBER){
					if(hand.get(z).getColor() == colorOfUpCard){
						return z;
					}
					if(hand.get(z).getNumber() == numOfUpCard){
						return z;
					}
				}
